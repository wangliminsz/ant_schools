...
export default {
    ...
    created: async function () {
        ...
        if (this.currentUserId) {
            try {
                this.loading = true;
                let response = await ContactService.getSchoolSpec_pattaya(
                    this.contactId
                );
                if (response) {
                    ...
                    let res_html = await ContactService.getSchoolHtml_pattaya(this.contactId)
    ...
    methods: {
        ...
        backSchool() {
            this.$router.push('/schooltable_pattaya');
        },
        ...
        async myHtml() {
            ...
            if (this.currentUserId) {
                ...
                try {
                    ...
                    let response = await ContactService.airUpdateHtml_pattaya(this.contact_html, this.contactId);
                    ...
                    if (response) {
                        ...
                        // this.$router.push('/')
                        const currentRoute = this.$router.currentRoute;
                        if (currentRoute.path !== "/schooltable_pattaya") {
                            this.$router.push("/schooltable_pattaya"); // Navigate to the page
                        }

                    } else {
                        this.loading = false;
                        const currentRoute = this.$router.currentRoute;
                        if (currentRoute.path !== `/schoolhtml_pattaya/${this.contactId}`) {
                            this.$router.push(`/schoolhtml_pattaya/${this.contactId}`); // Navigate to the page
                        }
                    }
                } catch (error) {
                    this.errorMessage = error;
            ...
            if (this.currentUserId) {
                ...
                try {
                    ...
                    if (response) {
                        ...
                        // this.$router.push('/schoolimgs/add/:contactId')
                        const currentRoute = this.$router.currentRoute;
                        if (currentRoute.path !== `/schoolimgs_pattaya/add/${this.contactId}`) {
                            this.$router.push(`/schoolimgs_pattaya/add/${this.contactId}`); // Navigate to the page
                        }
                    }
                } catch (error) {
                    this.errorMessage = error;
