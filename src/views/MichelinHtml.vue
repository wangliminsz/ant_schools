        ...
        if (this.currentUserId) {
            try {
                ...
                if (response) {
                    ...
                    let res_html = await ContactService.getMichelinHtml(this.contactId)
        ...
        async myHtml() {
            ...
            if (this.currentUserId) {
                ...
                try {
                    ...
                    let response = await ContactService.airUpdateMichelinHtml(this.contact_html, this.contactId);
                    console.log("GPT Michelin air update html response------------>", response);
                    ...
                    if (response) {
                        ...
                        this.loading = false;
                        const currentRoute = this.$router.currentRoute;
                        if (currentRoute.path !== `/michelinhtml/${this.contactId}`) {
                            this.$router.push(`/michelinhtml/${this.contactId}`); // Navigate to the page
                        }
                    }
                } catch (error) {
                    this.errorMessage = error;
