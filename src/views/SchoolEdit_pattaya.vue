    ...
    async myUpdate() {
      ...
      if (this.currentUserId) {
        try {
          ...
          let response = await ContactService.airUpdateRecord_pattaya(
            this.contact,
            this.contactId
          );
          ...
          if (response) {
            ...
            this.loading = false;
            const currentRoute = this.$router.currentRoute;
            if (currentRoute.path !== `/schooltable_pattaya/edit/${this.contactId}`) {
              this.$router.push(`/schooltable_pattaya/edit/${this.contactId}`); // Navigate to the page
            }
          }
        } catch (error) {
          this.errorMessage = error;
