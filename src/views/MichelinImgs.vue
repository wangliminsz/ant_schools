<template>
    ...
    <div v-if="!loading">
        <div class="mt-3 ml-5">
            <p style="font-size: 20px;">{{ contact.fields.name }}</p>
        </div>
    </div>
...
export default {
    ...
    created: async function () {
        ...
        if (this.currentUserId) {
            try {
                this.loading = true;
                let response = await ContactService.getMichelinSpec(
                    this.contactId
                );
                if (response) {
    ...
    methods: {
        ...
        async myAddImgs() {
            ...
            if (this.currentUserId) {
                ...
                // this.contact_img.Img_school_name = this.contact.fields.name
                ...
                const urlObjects = urls.map(img_imgur => ({ img_imgur, link_id: this.contactId }));
                console.log('Michelin Imgs --->>>', urlObjects)

                try {

                    // let response = await ContactService.createContact(this.contact)
                    let response = await ContactService.airInsertMichelinImg(urlObjects);
                    console.log("GPT Michelin add Imgs response------------>", response);
                    ...
                    if (response) {
                        ...
                        // this.$router.push('/')
                        const currentRoute = this.$router.currentRoute;
                        if (currentRoute.path !== "/mtable") {
                            this.$router.push("/mtable"); // Navigate to the page
                        }
                        ...
                        // this.$router.push('/schoolimgs/add/:contactId')
                        const currentRoute = this.$router.currentRoute;
                        if (currentRoute.path !== `/michelinimgs/add/${this.contactId}`) {
                            this.$router.push(`/michelinimgs/add/${this.contactId}`); // Navigate to the page
                        }
                    }
                } catch (error) {
                    this.errorMessage = error;
